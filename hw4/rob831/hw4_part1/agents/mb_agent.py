...
class MBAgent(BaseAgent):
    ...
    def train(self, ob_no, ac_na, re_n, next_ob_no, terminal_n):
        ...
        for i in range(self.ensemble_size):
            ...
            batch_size = max(1, num_data_per_ens)
            batch_indicies = np.random.choice(num_data, batch_size, replace=False)
            
            observations = ob_no[batch_indicies]
            actions = ac_na[batch_indicies]
            next_observations = next_ob_no[batch_indicies]
